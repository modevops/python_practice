...
def main():
    ...
    try:
        ...
        if e.resonse['Error']['Code']=="EntityAlreadyExists":
            print "Already Iam User with {} is exist".fromat(Iam_user_name)
            sys.exit(0)
        else:
            print "arn:aws:iam::aws:policy/AdministratorAccess"
            print e
            sys.exit(0)
    response = iam_client.create_access_key(UserName=Iam_user_name)
    print "Iam Useer Name={}".format(Iam_user_name)
    print "AccessKeyId={}".format(Iam_user_name)
